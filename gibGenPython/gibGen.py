...
for i in range(0, genNumPhonemes):
    ...
    for pair, idx in data:
    	...
    	if partA == mustStartWith:
            # FREQUENCYSUM totals number part of pair through loop of (data) pairs starting with a given letter.
    		frequencySum = (frequencySum + int(idx)); tmpTuple = [pair, frequencySum]; firstCharMatchedList.append(tmpTuple); logFile.write('appended partA value \'' + partA + '\' from pair \'' + pair + '\' with frequency ' + idx + '\nfrequencySum: ' + str(frequencySum) + '\n')
    ...
    if frequencySum != 0:
        ...
        for idx, pair in enumerate(firstCharMatchedList):
            ...
            if pair[1] >= PRND:
                pickedPair = str( firstCharMatchedList[ (idx - 1) ] ); nextLetter = pickedPair[3]; logFile.write('! ----------- PICK \'' + nextLetter + '\' for pair ' + pickedPair + ' at num ' + str(pair[1]) + '!\n'); break
        ...
        print('YO!')
        nextLetter = ' '; dataLen = len(data); PRND = randint(0, (dataLen - 1) ); logFile.write('No match starts with \'' + mustStartWith + '\'; terminated word and picking mustStartWith val from data set . . .\n'); mustStartWith = data[ (PRND) ][0][0]; logFile.write('PICKED new value \'' + mustStartWith + '\' for mustStartWith.\n')
